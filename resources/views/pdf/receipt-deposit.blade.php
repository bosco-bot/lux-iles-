...
<head>
    ...
    <title>Reçu d'Arrhes - {{ $reservation->reservation_number }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <div class="receipt-title">REÇU D'ARRHES</div>
    <div class="receipt-number">{{ $documentNumber }}</div>

    <div class="amount-box">
        <div class="amount-label">Montant reçu</div>
        <div class="amount-value">{{ number_format($payment->amount ?? $reservation->deposit_amount, 2, ',', ' ') }} €</div>
    </div>
    ...
    <div class="section">
        ...
        <div class="info-row">
            <span class="info-label">Acompte reçu:</span>
            <span><strong>{{ number_format($payment->amount ?? $reservation->deposit_amount, 2, ',', ' ') }} €</strong></span>
        </div>
        <div class="info-row">
            <span class="info-label">Solde restant:</span>
            <span>{{ number_format($reservation->balance_amount, 2, ',', ' ') }} €</span>
        </div>
    </div>
    ...
    <div class="footer">
        ...
        <p style="margin-top: 10px; font-size: 8pt;">Ce reçu certifie la réception de l'acompte pour la réservation #{{ $reservation->reservation_number }}</p>
    </div>
</body>
</html>
